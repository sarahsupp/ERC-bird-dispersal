...
for (g in grid_list){
  ...
  poly_groups <- grid %>% group_by(geometry) %>% 
    summarise(total_count=n(), 
              ...
              median_count = median(mean_weighted_count),
              .groups = 'drop') %>%
    as.data.frame()
...
```{r}
ggplot() +
  geom_point(data = poly_2022, aes(x = median_count, y = mean_mean_count))
```
...
for (p in poly_groups_list){
  ...
  print(
    ggplot() + 
      geom_sf(data=state_prov, colour="grey50", fill="white") + 
      geom_sf(data = poly_groups, aes(fill = median_count), color = NA) +
      scale_fill_viridis(option = "D") +
      #ggtitle("") +
      theme_void()
  )

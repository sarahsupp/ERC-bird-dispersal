...
#need to do maps for that are split up by month to avoid these kinds of results
#right now this map has every observation from 2022
...
#count/effort for each species separately in each cell, take mean of those numbers for each cell 
```
...
ggplot() + 
  geom_sf(data=state_prov, colour="grey50", fill="white") + 
  geom_sf(data = poly_groups_sf, aes(fill = total_count), color = NA) +
  scale_fill_viridis(option = "D") +
  theme_void() 

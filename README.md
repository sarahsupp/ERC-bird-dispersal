...
| migration-paths.Rmd | conteThis file is used analyze ebird data across multiple species. It creates average migration pathway, identifies start and end of migration seasons, and peak latitude (summer), and identifies linear trends in migration dates (onset, end) through time, where they exist. | outputs figures and results |
...
| ERC_shapefile_basics.Rmd | This file uses a shapefile of the ERC's range (W. Haas) to filter eBird data to only include observations that fall within that range | processes data |
...
Most of these files have been moved into the old-code-data folder for now

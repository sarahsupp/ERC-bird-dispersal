...
#FIXME: optimize this code to run in a vectorized way for speed and efficiency
```{r}
start1 <- Sys.time()
#Putting species, year, and days into list in order
species_list <- unique(df$species)
year_list <- sort(unique(df$year))
day_list <- sort(unique(df$day))
...
sample <- df %>% 
  ...
  summarise(numcells = n(), numobs = sum(count))
...
New way, check sys.time #its a win! 2x as fast
```{r}
start2 <- Sys.time()

# Sort unique values of year and day (optional if already ordered in df)
year_list <- sort(unique(df$year))
day_list <- sort(unique(df$day))

# Calculate weighted means for each combination of species, year, and day
weighted_mean_locs2 <- df %>%
  group_by(species, year, day) %>%
  summarize(
    weighted_lon = weighted.mean(newx, freq),
    weighted_lat = weighted.mean(newy, freq),
    weighted_z = weighted.mean(newz, freq))
...
%>%
  # Convert to decimal degrees
  mutate(
    mean_cal = map2(weighted_lon, weighted_lat, ~ deg(n_E2lat_lon(c(.x, .y, weighted_z)))),
    weighted_lon = map_dbl(mean_cal, 2),  # Extract longitude from mean_cal
    weighted_lat = map_dbl(mean_cal, 1)   # Extract latitude from mean_cal
  ) %>%
  dplyr::select(-mean_cal, -weighted_z)  # Drop temporary columns

,
    .groups = "drop"  # Ungroup after summarizing
  ) 

wml <- weighted_mean_locs2

coords <- data.frame(wml$weighted_lon, wml$weighted_lat, wml$weighted_z)
wml$lon2 <- deg(n_E2lat_lon(as.vector(coords[1,])))[1] #degree func

wml <- weighted_mean_locs2 %>%
  # Convert to decimal degrees
  mutate(
     mean_cal = deg(n_E2lat_lon(c(as.numeric(weighted_lon), as.numeric(weighted_lat), as.numeric(weighted_z))))[1]
  )
    
    #mean_cal = map2(as.numeric(weighted_lon), as.numeric(weighted_lat),
                    ~ deg(n_E2lat_lon(c(.x, .y, as.numeric(weighted_z))))),
    #weighted_lon = map_dbl(deg(n_E2lat_lon(c(weighted_lon, weighted_lat, weighted_z))))[1]
    weighted_lon = map_dbl(mean_cal, 2),  # Extract longitude from mean_cal
    weighted_lat = map_dbl(mean_cal, 1)   # Extract latitude from mean_cal
  )# %>%
  dplyr::select(-mean_cal, -weighted_z)  # Drop temporary columns

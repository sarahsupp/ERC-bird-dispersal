...
* Converting 6369952 rows into n-vectors roughly takes 21hours on the desktop machine in the CS dept. It does not run on a basic laptop.
* After 30 hours on Linux machine, was only 22% complete
* Recommend running this part of the code on a remote server for best results.
...
#FIXME: df should be the same number of rows as dat_effort
...
#FIXME: Delete the below commented chunk, if the above works. I think it should reduce total compute time at least threefold!
# for (i in 1:nrow(df)){
#   new_vals <- lat_lon2n_E(rad(df[i, "y"]), rad(df[i,"x"]))
#   df[i,"newx"] <- new_vals[1]
#   df[i,"newy"] <- new_vals[2]
#   df[i,"newz"] <- new_vals[3]
# }

...
for (i in 1:length(species_list)){
  ...
  for (j in 1:length(year_list)){
    ...
    for(k in 1:length(day_list)){
        ...
        weighted_mean_locs2 <- rbind(weighted_mean_locs2, 
...
# counts the number of unique cells (numcells) and the total number of checklists (numobs)
daily_counts <- df %>% 
  group_by(species, year, day) %>% 
  summarise(numcells = n(), numobs = sum(count)) %>%
  ungroup()

#append the counts to the weighted locs dataframe

...
Use GAM model to predict daily location along a smoothing line, from the weighted mean locations. Daily location should be calculated for each year separately, but could probably be written using dplyr or apply (purr??) to simplify and optimize (used a series of for loops in 2014 code instead).
...
EstimateDailyLocs = function(dat) {
  ...
  preds =  data.frame(DAY = xpred$DAY, pred_lon = lonpred$fit, 
                      pred_lat = latpred$fit, lon_se = lonpred$se.fit, lat_se = latpred$se.fit)
...
ggplot(dailylocs, aes(DAY, pred_lat)) + geom_line(size=1) + 
  geom_vline(xintercept=c(101, 209, 362), col="gray30") + #FIXME: Update xintercepts
  xlab("Day of Year") + ylab("predicted latitude centroid") +
  theme_bw()

#cool! Spring migration really stands out at the widely spaced dots. This means the species is moving rapidly across the landscape during these 2-3 weeks.
ggplot(dailylocs, (aes(pred_lon, pred_lat))) + geom_point() + 
  xlab("predicted latitude centroid") + ylab("predicted longitude centroid") +
  theme_bw()

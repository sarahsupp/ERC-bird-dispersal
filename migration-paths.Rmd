...
weighted_mean_locs <- dat_effort %>%
  ...
  summarise(
            numcells = n(), 
            numobs = sum(count.x),
            wtmean_lon = wt.mean(lon, count.x/count.y), #FIXME
...
grid <- merge(dat_effort, grid, by.x="POLYFID", by.y="cell")
#Get polygons for the spatial range and make a map of bird observations
countries <- map_data("usa") 

ggplot() + 
 # geom_polygon(data=countries, aes(x=long, y=lat, group=group), fill=NA, color="black")   +
  geom_polygon(data=grid,      aes(x=lon, y=lat.x, group=group, fill=count.x), alpha=0.4)    +

...
#converting x and y to newx, newy, newz(radius)
for (i in 1:nrow(df)){
...
#creating a dataframe for average location (calculating weighted means for newx, newy, newz)
# weighted_mean_locs <- matrix(NA, nrow = 5490, ncol = 4)
# colnames(weighted_mean_locs) <- c("year",'day','meanx','meany')
...
for (i in 1:length(species_list)){
  ...
  for (j in 1:length(year_list)){
    ...
    for(k in 1:length(unique(year_df$day))){
        ...
        # weighted_mean_locs[c(366*(j-1)+k),"meanx"] <- mean_cal[2]
        # weighted_mean_locs[c(366*(j-1)+k),"meany"] <- mean_cal[1]
        # weighted_mean_locs[c(366*(j-1)+k),"day"] <- k
        # weighted_mean_locs[c(366*(j-1)+k),"year"] <- year_list[j]
    }
  }
}  

# weighted_mean_locs <- matrix(NA, nrow = 76860, ncol = 5)
# colnames(weighted_mean_locs) <- c('species','year','day','meanx','meany')
# 
...
#         mean_cal <- deg(n_E2lat_lon(mean_cal)) #degree func
# 
#         weighted_mean_locs[c(k),"meanx"] <- mean_cal[2]
#         weighted_mean_locs[c(k),"meany"] <- mean_cal[1]
#         weighted_mean_locs[c(k),"species"] <- species_list[i]
#         weighted_mean_locs[c(k),"year"] <- year_list[j]
#         weighted_mean_locs[c(k),"day"] <- day_list[k]
#       }
#   }
# }  
...
# summaries part goes here


# weighted_mean_locs <- df %>%
...
#weighted_mean_locs <- df %>% select("species", "year", "day")
...
# weighted_mean[c(i),"meanx"] <- weighted_mean[2]
#   weighted_mean[c(i),"meany"] <- weighted_mean[1]
# weighted_mean_locs <- df %>%
#     mutate(day = as.numeric(day),
#          year = as.numeric(year)) %>%
# group_by(species, year, day) %>%
...
# spp <- unique(dat_effort$species)
# days <- unique(dat_effort$day)
# years <-- unique(dat_effort$year)
...
mggplot(dailylocs, aes(DAY, lat)) + geom_line(size=1, aes(col=species)) + 

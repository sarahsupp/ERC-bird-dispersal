...
# save(dat_effort, file = "data/dat_effort_ROBI.RData")
```
...
Est3MigrationDates = function(dat){
  ...
  for (i in year){
   dat_subset <- 
    ...
    spring_threshold = min(dat_subset$lat_se[c(1:120)]*2.56 + dat_subset$lat[c(1:120)])
    fall_threshold = min(dat_subset$lat_se[c(280:364)]*2.56 + dat_subset$lat[c(280:364)])
    ...
    dates = data.frame(YEAR = i, SPRING = spring_begin, MAXLAT = max_lat, FALL = fall_end)

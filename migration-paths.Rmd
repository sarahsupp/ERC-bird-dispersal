...
```{r} 
#FIXME: count.x is just x? count.x = count 

#z = radius not altitude 
...
weighted_mean_locs <- df %>%
   ...
   #group_by(species, year, day) # group by working?????? not sure..need to be checked... help..once we run the group_by and summarise, it only shows the species, numcells, numobs
#   %>%
...
weighted_mean_locs$long <- NA
weighted_mean_locs$lat <- NA


for(i in 1:nrow(weighted_mean_locs)){
    
  cnt <- c(weighted.mean(weighted_mean_locs$newx, weighted_mean_locs$freq),
           weighted.mean(weighted_mean_locs$newy, weighted_mean_locs$freq),
           weighted.mean(weighted_mean_locs$newz, weighted_mean_locs$freq))
  
    
  cnt <- deg(n_E2lat_lon(cnt)) #degree func
  
  weighted_mean_locs[c(i),"long"] <- cnt[2]
  weighted_mean_locs[c(i),"lat"] <- cnt[1]
  }


# put this in for loop 
# 
# wtmean_lon = deg(n_E2lat_lon((c(
#               weighted.mean(newx, freq), 
#               weighted.mean(newy, freq),
#               weighted.mean(newx, freq),
#             ))))[2]
# 
# wtmean_lat = deg(n_E2lat_lon((c(
#               weighted.mean(newx, freq), 
#               weighted.mean(newy, freq),
#               weighted.mean(newx, freq),
#             ))))[1]

...
Aarrgggh rewrite the code to fix what I lost when R froze. 
```{r}
# look at the structure of sppR_mo

list unique cells, 
list unique months
empty df with cell, mo, n, slope, pVal -- maybe also retain lat and long
...
dat 2
if nrow(dat2) >=3
mod=lm(Smed~as.numeric(YEAR))
n=
  slope=
  pVal=
  sig=ifelse(pVal < 0.05, "sig", "NS")
  
  df = rbind(df, c(c, m, n, slope, pVal, sig))
  
# adding factor names and levels to month, ordered by winter season, not calendar year
df$monthname <- month(df$month, label=TRUE)
df$monthname <- factor(df$monthname, levels=c("Oct", "Nov", "Dec", "Jan", "Feb", "Mar"))
  
df <- df %>% filter(!is.na(sig)) %>%
  filter(m!=0)
  
#ggplot histogram, layered by identity for sig and NS

# ggplot boxplot by month for all significant slopes, included a wilcoxon.test sig for oct vs nov, jan, feb
# wanted to save this as transparent for ESA poster

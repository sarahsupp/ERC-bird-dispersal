...
#library(rnaturalearthhires)package ‘rnaturalearthhires’ is not available (for R version 3.6.0)
library(lubridate)
```
...
Est3MigrationDates = function(dat){
  ...
  for (i in x){
   sub_dailylocs <- 
    ...
    spring_threshold = min(dat$lat_se[c(1:120)]*2.56 + dat$lat[c(1:120)])
    fall_threshold = min(dat$lat_se[c(280:365)]*2.56 + dat$lat[c(280:365)])
    spring_index = 11:190
    fall_index = 220:355
    spring_max = spring_index[which.max(dat$lat[spring_index])]
    fall_max = fall_index[which.max(dat$lat[fall_index])]
    
    #identify beginning of spring migration
    tst = 1000
    spring_index2 = spring_max
    while(tst > spring_threshold){
      tst = dat$lat[spring_index2]
...
dates <- Est3MigrationDates(weighted_mean_locs) #FIXME : The input here should be the daily locs from the GAM

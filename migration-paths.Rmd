...
#library(rgdal) #FIXME: rgdal was removed from CRAN. Is it needed for this code? Check, and remove if not
#library(rgeos) #FIXME: rgeos was removed from CRAN. Is it needed for this code? Check, and remove if not
...
# library(SDMTools)#FIXME: SDMTools removed from CRAN. Is it needed for this code? Check, and remove if not
```
...
Note: If data has not been updated (~2022~.), then go straight to "weighted mean location" chunk and load "df_nvector.rds" file. If the data has been updated then have to start from the beginning (loading spdata)
...
Note: After running "process-raw-eBrid-data" and re-run the *spdata*, the names of the variables (columns names) are changed. For example, in num_spobs function, count.x is not found in dat_effort. YEAR and DAYS have to be change into lower case letter. 
...
```
*Note that the total number of observations decreases for 2023 because the data was downloaded through May 23, 2023, so it is not a complete calendar year.
...
## FIXME: Would it make sense to reorganize this code so it doesn't run in this RMD doc but is in a separate file?
...
If the data was updated then start the whole code chunk. (converting 6369952 rows into n-vectors roughly takes 21hours). 
Made an copy of original file just to be safe. "dat_effort = df"

```{r} 
#copying the raw data(dat_effort)
...
# #Save to RDS file
#saveRDS(df, "df_nvector_2023.rds")
```


## If df_nvector_2023.rds already exists, JUMP TO THIS CODE CHUNK.
Loads the processed data
```{r}
# #Loading RDS file 
df <- readRDS("data_2023/df_nvector_2023.rds")
```
...
ggplot(weighted_mean_locs, aes(day, weighted_lon)) + geom_point(alpha=0.10) + facet_wrap(~species, ncol=3) + theme_bw()
ggsave(filename = "figs/15_species_mean_locs_lon.png", height = 10, width=8)

ggplot(weighted_mean_locs, aes(day, weighted_lat)) + geom_point(alpha=0.10) + facet_wrap(~species, ncol=3) + theme_bw()
...
## If you are starting here, can upload the .rds file directly rather than running the previous code up to this point.
```{r}
# #Loading RDS file 
...
ggsave(filename = "figs/15species_weighted_mean_locs_smmooth_2023.png", height = 11, width=8)
```
...
#filtering only the migratory species (9 species)
  #Sturnella magna and Sturnus vulgaris are not working (magna not working in 2017, vulgaris not working in 2008)
...
ggplot(spp7dailylocs, aes(as.numeric(day), distance, group=year)) + geom_line(aes(col=species)) + 
...
ggsave(filename = "figs/7species_distances_2023.png", height = 5, width=8)


#Dr. Supp original code 
# #identify dates with no location and assign all values to NA
# missdates <- dailylocs %>%
#   group_by(species, year) %>%
#   #technically this leaves out DAY 366 in 2008, 2012, 2016, and 2020, but I don't think that matters?
...
#TODO: filter out the non-migratory species when plotting 
...
# ggsave(filename = "figs/15species_distances.png", height = 5, width=8)
```
...
```{r}
#originallly dailylocs has 9 colums and spp7dailylcs has 10 columns. In order to run the function, we need to have the same column. 
...
EstimateMaxSpeed = function(dat) {
  ...
  df_maxspeed <- data.frame(species = "none", year = 0, max_speed = 0)
  for (s in species){
    for (i in years){
   sub_dailylocs <- 
       spp7dailylocs %>%
...
ggplot(spp7max_speed, aes(year, max_speed)) + geom_line(aes(col=species)) +
...
ggplot(spp7max_speed, aes(year, max_speed)) + geom_point() +
...
ggsave(filename = "figs/7species_MedMaxMigrationSpeed_2023.png")


# # function to estimate the maximum speed of migration (km/day) for all 15 species
# EstimateMaxSpeed = function(dat) {
#   dat <- dat %>%
#   filter(day != 1)
#   years <- c(2008:2022)
...
#   )
# 
# #TODO: filter only the migratory species 
# #plot the results estimating yearly maximum migration speed (km/day)
...
#   facet_wrap(~species)
# 
# ggsave(filename = "figs/15species_MedMaxMigrationSpeed.png")
```
...
Note: Since 2022 is only half way filled, year of 2022 is excluded in Est3MigrationDates. 
...
Est3MigrationDates = function(dat){
  ...
  year <- c(2008:2022)
  species <- unique(dat$species)
  df_dates <- data.frame(species="none", year=0, spring = 0, maxlat = 0, fall = 0)
  for (s in species) {
    for (y in year){
      dat_subset <- 
        dat %>%
...
Looking at the linear trends of these dates for each 7 migratory species to see if there are any significance. 
```{r}
...
purpureus <- spp7MigrationDates %>%
  filter(species=="Haemorhous purpureus")

purpureus_spring <- lm(spring ~ year, data=purpureus)
#summary(purpureus_spring)

purpureus_fall <- lm(fall ~ year, data=purpureus)
#summary(purpureus_fall)

# #anova 
# anova(mod2_spring)
# anova(mod2_fall)

mustelina <- spp7MigrationDates %>%
  filter(species=="Hylocichla mustelina")

mustelina_spring <- lm(spring ~ year, data=mustelina)
#summary(mustelina_spring)

mustelina_fall <- lm(fall ~ year, data=mustelina)
#summary(mustelina_fall)

# #anova
# anova(mod3_spring)
# anova(mod3_fall)

hyemalis <- spp7MigrationDates %>%
  filter(species=="Junco hyemalis")

hyemalis_spring <- lm(spring ~ year, data=hyemalis)
#summary(hyemalis_spring)

hyemalis_fall <- lm(fall ~ year, data=hyemalis)
#summary(hyemalis_fall)

# #anova 
# anova(mod4_spring)
# anova(mod4_fall)

coronata <- spp7MigrationDates %>%
  filter(species=="Setophaga coronata")

coronata_spring <- lm(spring ~ year, data=coronata)
#summary(coronata_spring)

coronata_fall <- lm(fall ~ year, data=coronata)
#summary(coronata_fall)

# #anova
# anova(mod5_spring)
# anova(mod5_fall)

sialis <- spp7MigrationDates %>%
  filter(species=="Sialia sialis")

sialis_spring <- lm(spring ~ year, data=sialis)
#summary(sialis_spring)

sialis_fall <- lm(fall ~ year, data=sialis)
#summary(sialis_fall)
...
migratorius <- spp7MigrationDates %>%
  filter(species=="Turdus migratorius")

migratorius_spring <- lm(spring ~ year, data=migratorius)
#summary(migratorius_spring)

migratorius_fall <- lm(fall ~ year, data=migratorius)
summary(migratorius_fall)
...
```{r}
#making linear regression result table for spring 
stargazer(cedrorum_spring, coronata_spring, migratorius_spring, mustelina_spring, purpureus_spring, hyemalis_spring, sialis_spring, type = "text",title = "Spring Migration Species Directional Changes", column.labels=c("B.cedrorum","S.coronata","T.migratorius", "H.mustelina","H.purpureus","J.hyemalis","S.sialis"), out = "figs/Spring Migration Species Directional Changes.txt", covariate.labels = c("year", "Intercept") ,report = "vcp*")
...
df_sppR <- data.frame(cell=0, month=0, n=0, slope=0, pVal=0, cell_lat=0, cell_lon=0)

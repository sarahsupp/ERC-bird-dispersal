...
Can use the winter column to grab all data October-March and the latitudes ~ 26-47 _Not that these choices are somewhat arbitrary, just to get a quick look now. Will need to get finer grained info from D. Ward and lab later...
```{r}
ERCbirds <- dailylocs %>%
...
ggplot() + 
  ...
  geom_point(data=ERCbirds, aes(lon, lat), alpha=0.10, col="red") + 
  xlim(-102,-52) + 
  theme_bw() + 
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
```{r}
my_spdf <- readOGR( 
  dsn=paste0(here(), "/data/shapefile/") , 
  layer="litt68av",
  verbose=FALSE
)
...
par(mar=c(0,0,0,0))
plot(my_spdf, col="darkolivegreen", bg="grey80", lwd=0.25, border=0 )

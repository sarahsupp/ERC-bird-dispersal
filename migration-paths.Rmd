...
```{r}
#TODO: Find a better way to automate this data prep part so it can automatically read the data, add species names, and prepare it
# raw species data, queried by FAL
# load("data/Bombycilla_cedrorum.RData") # will come in as ebrd2.c, FAL named previously
# load("data/Cyanocitta_cristata.RData")
# load("data/Hylocichla_mustelina.RData")
# load("data/Turdus_migratorius.Rdata")
# load("data/Setophaga_coronata.Rdata")

# raw eBirder data, queried by FAL
 load("data/effort2020.Rdata") # will come in as eft.c, FAL named previously
 load("data/locs_2020.Rdata") # will come in as locs, FAL named previously
# 
...
# # #now merge with the third data file for effort 
  ...
  save(dat_effort, file = "data/dat_effort_BLJA.RData")
```

**If you have already completed the initial steps** and have saved out the effort file, start here instead, with loading it.
Each species has it's own merged data file, and needs to be dealt with separately (for now) to get the results.
```{r}
files <- list.files(path = "data/", pattern = "dat_effort*")
files <- paste0("data/", files)
combined_files <- bind_rows(lapply(files, fread)) # FIXME

#load("data/dat_effort_ROBI.RData")
```
...
paste0("The total number of observations is ", num_obs, ".")
```
...
ggsave(filename = "figs/YEWA_obs.png", height = 3, width=8)
```
...
p <- ggplot() +
  geom_polygon( data=state_prov, aes(x=long, y=lat, group = group), colour="black", fill="white" ) +
...
Estimatedailylocs = function(dat) {
  ...
  df <- data.frame(DAY = 0, YEAR =0, lon = 0, lat = 0, lon_se = 0, lat_se = 0)
  ...
  for (i in x) {
   ...
   preds =  data.frame(DAY = xpred$DAY, YEAR = i, lon = lonpred$fit, 
...
distances <- append(NA, distVincentyEllipsoid(as.matrix(dailylocs[,c(3,4)])))/1000

# add column to original dataset
dailylocs$distance <- distances

#Remove Day 1 for all year, because we know, due to our method of separating GAM by year, it will be a flawed value
dailylocs <- dailylocs %>%
  filter(DAY != 1)

#TODO: If placed after the dates are calculated, then the speeds used can be limited to the migraiton seasons using the filter below. Not sure we need to do that here, but I think this is how I got around the year-to-year day 1 being wrong things last time, plus, that project was really only focused on th e migration seasons alone.
#  filter(DAY >= migration_dates$spring_begin & migration_dates <= migration_dates$autumn_end)

#FIXME: 
ggplot(dailylocs, aes(as.numeric(DAY), distance, group=YEAR)) + geom_line() + 
...
EstimateMaxSpeed = function(dat) {
  ...
  filter(DAY != 1)
  years <- c(2008:2019)
  df <- data.frame(YEAR = 0, MAX_SPEED = 0)
  for (i in years){
   sub_dailylocs <- 
       dailylocs %>%
       filter(YEAR == i)
    ...
    max_speed <-  data.frame(YEAR = i, MAX_SPEED = median)
...
mean(max_speed$MAX_SPEED)
sd(max_speed$MAX_SPEED)
med <- median(max_speed$MAX_SPEED)

#plot the results estimating yearly maximum migration speed (km/day)
ggplot(max_speed, aes(YEAR, MAX_SPEED)) + geom_line() + 
...
Ask Dr. Supp if you have questions :)
```{r}
...
Est3MigrationDates = function(dat){
  ...
  filter(DAY != 1)
  year <- c(2008:2019)
  df <- data.frame(YEAR=0, SPRING = 0, MAXLAT = 0, FALL = 0)
  for (i in 1:length(year)){
   ...
   # ALSO ADDED NA.RM=TRUE ARGUMENT, DOES THIS HAVE ANY DOWNSIDES?
    ...
    dates = data.frame(YEAR = year[i], SPRING = spring_begin, MAXLAT = max_lat, FALL = fall_end)
...
begin_winter <- median(dates$FALL)
end_winter <- median(dates$SPRING)
...
ggsave(filename = "figs/WOTH_winterDates.png", height = 4, width=8)
```

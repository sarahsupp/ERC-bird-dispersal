...
ggplot(df %>% filter(sig=="sig"), aes(monthname, slope)) + 
  geom_boxplot(fill="black", col="gray40", notch=TRUE) +
  geom_jitter(col="darkturquoise", alpha=0.5) +
  geom_hline(yintercept=0, linetype="dashed", col="gray40") +
    theme(
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill = "transparent",colour = NA),
...
# Figure for ESA 2021 poster
ggsave("figs/Smed_slopes_month.png", bg="transparent", height=5, width=11)

...
weighted_mean_locs <- dat_effort %>%
  ...
  summarise(
            numcells = n(), 
            numobs = sum(count.x),
            wtmean_lon = weighted.mean(lon, count.x/count.y), #FIXME: New way to calculate weighted count?
            wtmean_lat = weighted.mean(lat, count.x/count.y)#, 
            #wtsd_lon <- weighted.sd(lon, count.x/count.y),
            #wtsd_lon <- weighted.sd(lat, count.x/count.y) FIXME: standard deviation
            )
```

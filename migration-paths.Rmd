...
files = list.files(path=here("data/effort-merged_data_2023/"), pattern=glob2rx("dat_effort*.rds"))

spdata <- data.frame(POLYFID=0, year=0, day=0, count=0, x=0, y=0, effort=0, species="none")
for (i in files) {
  dat <- readRDS(paste0(here("data/effort-merged_data_2023/"),(i)))
...
Note: After running "process-raw-eBrid-data", then load the 'df_nvector.rds'. 
If the data was updated then start the whole code chunck. (converting 6369952 rows into n-vectors roughly takes 21hours). 
...
#ggsave(filename = "figs/15_species_mean_locs_lon.png", height = 11, width=8)
ggplot(weighted_mean_locs, aes(day, weighted_lat)) + geom_point(alpha=0.10) + facet_wrap(~species)
#ggsave(filename = "figs/15_species_mean_locs_lat.png", height = 11, width=8)
...
#saveRDS(weighted_mean_locs, "weighted_mean_locs.rds")
...
ggsave(filename = "figs/15species_weighted_mean_locs_smmooth.png", height = 11, width=8)
```
...
spp7dailylocs <- dailylocs %>% filter (species %in% c("Bombycilla_cedrorum", "Haemorhous_purpureus", "Hylocichla_mustelina", "Junco_hyemalis", "Setophaga_coronata", "Turdus_migratorius", "Sialia_sialis"))
...
ggsave(filename = "figs/7species_distances.png", height = 5, width=8)
...
ggsave(filename = "figs/7species_MedMaxMigrationSpeed.png")
...
#saveRDS(spp7MigrationDates, "spp7MigrationDates.rds")
...
ggsave(filename = "figs/7spp_MigrationDates.png", height = 10, width=10)
```
...
purpureus <- spp7MigrationDates %>%
  filter(species=="Haemorhous_purpureus")
...
mustelina <- spp7MigrationDates %>%
  filter(species=="Hylocichla_mustelina")
...
coronata <- spp7MigrationDates %>%
  filter(species=="Setophaga_coronata")
...
migratorius <- spp7MigrationDates %>%
  filter(species=="Turdus_migratorius")

...
Est3MigrationDates = function(dat){
  ...
  for (s in species) {
   ...
   # ALSO ADDED NA.RM=TRUE ARGUMENT, DOES THIS HAVE ANY DOWNSIDES?
    ...
    a<- which(dat$winter == 'non-winter')

    first_non_winter=a[1]

    #first_non_winter #row number = 1

    b<- last_non_winter=nrow(dat)
    
    #last_non_winter #row number = last row number

    c<- first_winter = 1
    #first_winter
    d<- last_winter = first_non_winter - 1
    #last_winter
    
    
    spring_threshold = min(filter(dat_subset, day %in% c(a:b))$lat_se*2.56 + filter(dat_subset, day %in% c(a:b))$lat, na.rm=TRUE)
    fall_threshold = min(filter(dat_subset, day %in% c(c:d))$lat_se*2.56 + filter(dat_subset, day %in% c(c:d))$lat, na.rm=TRUE)

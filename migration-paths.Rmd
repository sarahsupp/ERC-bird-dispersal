...
weighted_mean_locs <- dat_effort %>%
  ...
  summarise(
            numcells = n(), 
            numobs = sum(count.x),
            wtmean_lon = wt.mean(lon, count.x/count.y), #FIXME: New way to calculate weighted count?
            wtmean_lat = wt.mean(lat, count.x/count.y), 
            wtsd_lon <- wt.sd(lon, count.x/count.y),
            wtsd_lon <- wt.sd(lat, count.x/count.y))
```
...
#Wdat_effort$cell_lon <-dgSEQNUM_to_GEO(dgg, dat_effort$cell)$lon_deg 

cellcenters   <- dgSEQNUM_to_GEO(dgg, dat_effort$POLYFID)
cewa_counts <- dat_effort %>%
  group_by(POLYFID) %>% 
  summarise(count=n())
ggplot(cewa_counts, aes(x=count)) + geom_histogram(binwidth=10)
```
...
grid <- dgcellstogrid(dgg, dat_effort$POLYFID, frame=TRUE, wrapcells=TRUE)

#Update the grid cells' properties to include the number of observations in each cell
grid <- merge(grid, cewa_counts, by.x="cell", by.y="POLYFID")
...
ggplot() + 
  ...
  #FIXME: something wrong with the long and lat of each hexagon
...
p <- ggplot() +
  ...
  geom_point(data=dat_effort, aes(x=lon, y=lat), alpha=0.25, size=0.5, col="mediumpurple")  + 

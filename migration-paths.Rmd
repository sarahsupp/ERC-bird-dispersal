...
Note: After running "process-raw-eBrid-data", re-run the *spdata*, copying *dat_effort*, start from line 174(readRDS"df_nvector").
If the data was updated then start the whole chunck. (converting 6369952 rows into n-vectors roufly takes 21hours). 
*dat_effort* = *df*
...
Should I filter out the non-migratory species?
```{r}
...
TODO: tyr to filter out the migratory species when plotting 
...
#filtering only the migratory species (9 species)
  #only 7 species are working, Sturnella magna and Sturnus vulgaris are not working (magna not working in 2017, vulgaris not      working in 2008)
...
ggplot(spp7max_speed %>% filter(species!="Cyanocitta cristata"), aes(year, max_speed)) + geom_line(aes(col=species)) +
...
```
make fig file in my github and change the name into more general 
...
winter and non-winter are for eastern read cedar tree (cone season)
day %in% c(1:151))-> refering julian day

These are spring index and fall index 
...
#TODO: Since 2022 is only half way filled, year of 2022 is not included
...
Est3MigrationDates = function(dat){
  ...
  for (s in species) {
   ...
   # ALSO ADDED NA.RM=TRUE ARGUMENT, DOES THIS HAVE ANY DOWNSIDES?
    ...
    #index: finding exact date of the migration date
    #TODO: (Dr. Supp needs to check) double check index dates 
...
TODO: 
linear regression 
plot (time, spring day )
slope: #days earlier/later migration per year 

regression example: Pr(>|t|) :0.415 if this p-value significant, then look at the slope -0.2857 (times 10)-> for decade, species started migration 2 days earlier 

*Eastern Bluebird: Sialia_sialis fall p-value significant*


Looking at the linear trends of these dates for 7 migratory species.
```{r}
...
stargazer(cedrorum_fall, coronata_fall, migratorius_fall, mustelina_fall, purpureus_fall, hyemalis_fall, sialis_fall, type = "text",  
     title = "Fall Migration Species Directional Changes")
```

```{r}
#making linear regression result table for spring 
stargazer(cedrorum_spring, coronata_spring, migratorius_spring, mustelina_spring, purpureus_spring, hyemalis_spring, sialis_spring, type = "text",  
     title = "Spring Migration Species Directional Changes")
```
...
#TODO: ANOVA -check out (grouping all the months )
#boxplot: all positive, the middle part 95% percentile (median)

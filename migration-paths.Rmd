...
grid <- dggridR::dgcellstogrid(dgg, spp_counts$cell) #-> working.. IS this correct?s
...
```
julian day_> check the month(convert the days)-> ~100~200 spring (highest peak)
first peak spring 
second peak winter 
...
```{r}
#filtering only the migratory species 
dailylocs <-dailylocs %>% filter (species %in% c("Bombycilla_cedrorum", "Haemorhous_purpureus", "Hylocichla_mustelina", "Junco_hyemalis", "Setophaga_coronata", "Sturnella_magna" ,"Sturnus_vulgaris", "Turdus_migratorius", "Sialia_sialis") )

#dailylocs2 <-dailylocs %>% filter (species %in% c("Bombycilla_cedrorum", "Haemorhous_purpureus") )
```


```{r}
# modified from old function Est3MigrationDates in migration-fxns.R
#TODO: need to figure out for year 2022
Est3MigrationDates = function(dat){
  ...
  for (s in species) {
   ...
   # ALSO ADDED NA.RM=TRUE ARGUMENT, DOES THIS HAVE ANY DOWNSIDES?
    ...
    spring_threshold = min(filter(dat_subset, day %in% c(2:120))$lat_se*2.56 + filter(dat_subset, day %in% c(2:120))$lat, na.rm=TRUE) # 1:120 Jan 1 ~ April 30
    fall_threshold = min(filter(dat_subset, day %in% c(244:364))$lat_se*2.56 + filter(dat_subset, day %in% c(244:364))$lat, na.rm=TRUE) # Oct7 ~ Sep 1 244:364
    ...
    #190: july 9, index: finding exact date of the migration date
...
#filter out non-migratory species 
...
# winter_dat_effort_new <- dailylocs %>%
#   mutate(DAYm1 = day-1,
#          origin = paste0(year, "-01-01"),
#          date = as.Date(DAYm1, origin=origin), 
#          month = month(date)) %>%

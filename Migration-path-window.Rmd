...
maxmin_val_day <- merge(max_value, min_value, by = c("species", "year")) #combining max_value_day and mim_value_day data frames by species and year columns
maxmin_val_day
...
years <- sort(unique(Bombycilla_centroids$year)) #sorting the year 
...
for(jjj in dataresult){
  ...
  years <- 2009:2021 #need to take year from 2009 to 2021 so that it will include 2008 and 2022 as calculating the buffer year
  ...
  for(jjj in 1:length(years)) {
    ...
    filterdowntothreeyrs <- Bombycilla_centroids[Bombycilla_centroids$year %in% bufferyear,] #saving the buffer years
    if(nrow(filterdowntothreeyrs)==0) next #checking
  
    takeoutextraday <- filterdowntothreeyrs[filterdowntothreeyrs$day != 366,] #taking out an extra day 366, so that each year has 365 days 
    endofyear1winter <- takeoutextraday[takeoutextraday$year == bufferyear[1] & takeoutextraday$day>319,] # taking a first year and day above 319 
    ...
    focalyear <- takeoutextraday[takeoutextraday$year == bufferyear[2],] #this is the focal year (second year)
    focalyear$day2 <- focalyear$day
  
    beginningofyear3winter <- takeoutextraday[takeoutextraday$year==bufferyear[3] & takeoutextraday$day<74,] ## taking a third year and day above 74 

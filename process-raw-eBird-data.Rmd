...
# raw eBirder data, queried by FAL
 load("data/raw_eBird_data_2022/effort-count_2022.Rdata") 
 # (2020) will load as eft.c, FAL named previously
 # (2022) will load as dat, FAL named previously

 load("data/raw_eBird_data_2022/locs_2022.Rdata") 
 # (2020 & 2022) will load as locs, FAL named previously
```

This code chunk will identify all 15 species files, bring them in one by one, and loop through the merge and write process so that we don't have to repeat all the code.
```{r}
spfiles = list.files(path=here("data/raw_eBird_data_2022/species/"), pattern=glob2rx("*.RData"))

for (i in spfiles) {
  # These RData files have one object and it is named "dat2" 
  load(paste0(here("data/raw_eBird_data_2022/species/"),(i)))
  ...
  saveRDS(dat_effort, file = paste0("data/effort-merged_data/dat_effort_", spname, ".rds"))
}
```
...
For *cedar waxwing*
```{r}
load(here("data/raw_eBird_data_2022/species/Bombycilla_cedrorum.RData")) 
# (2020) will load as ebrd2.c, FAL named previously
# (2022) will load as dat2, FAL named in previous query

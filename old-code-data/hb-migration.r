...
# read in species data (one at a time, or write a script that can loop trhough them all)
...
hbird$year = factor(hbird$year, levels=c(2008:2018), ordered=TRUE) #set to factor instead of integer

#grab species name for setting directory paths and naming figures
species = hbird$scientific_name[1] 
...
# FIXME: In Supp et al., we used grids from Sahr et al at area=12,452 km2. 
#        To match this using dggrid, Use Fuller4H projection with res=6 (same resolution as other FAL papers)
dgg <- dgconstruct(projection = "FULLER", aperture = 4, topology = "HEXAGON", res=6)
...
daily_counts <- merge(daily_hex_counts, effort_one_year, by.x = c("cell", "DAY"), by.y = c("POLYFID", "DAY")) #FIXME: This won't work with the example data so I can't test it, POLYFID from old datasets has been renamed simply as "cell"

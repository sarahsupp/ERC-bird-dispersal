...
#BBL_all <- BBL_all[BBL_all$event_type == 'B',]
...
#also making list of scientific names
sci_birds <- unique(just_banding$species_scientific_name)

bird_counts <- list()
for (b in birds){
  count <- length(which(BBL_all$species_name == b))
...
BBL_year_count <- BBL_all %>% group_by(species_name, event_year) %>% 
...
^ this shows that there are, in fact, some recaptures that happen at different locations than the original, but as far as I can tell, all of these recaptures are still really close to each other

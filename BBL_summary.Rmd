...
Remove months that appear to be typos or represent something else (i.e., >12)
```{r}
...
results <- data.frame(
  ...
  distance_m = numeric(), north_south_m = numeric(), east_west_m = numeric(), 
...
for (id in unique(BBL_recaptures$original_band)) {
  ...
  for (i in 1:(nrow(id_data) - 1)) {
    ...
    results <- rbind(results, data.frame(
      ...
      distance_m = distance, north_south_m = lat_diff, east_west_m = lon_diff,
...
species_summaries <- results%>%
  group_by(species_scientific_name) %>%
  summarize(mean(distance_m), median(distance_m), min(distance_m), max(distance_m))
...
for (b in birds){
  ...
  print(ggplot() + 
    ...
    geom_path(data = df, aes(x = lon, y = lat, group=original_band), 
              col = "olivedrab",
              linewidth = 0.25, lineend = "round", alpha=0.25) +
    labs(x = "", y = "", title = b) +
    theme_minimal() +

...
effort_locs <- dplyr::inner_join(dat, locs2, by="POLYFID")
...
for (i in spfiles) {
  ...
  dat_merged = merge(dat2, locs, by.x = "POLYFID", by.y = "POLYFID")
  ...
  dat_effort = merge(dat_merged, dat, by.x = c("POLYFID", "year", "day"), by.y = c("POLYFID","year",  "day")) 
  ...
  spname <- sub("\\.[[:alnum:]]+$", "", i)
  dat_effort$species <- spname
  # save file, also adding the species name, into a new folder with the species + effort merged datasets
  saveRDS(dat_effort, file = paste0("data/2-intermediate_eBird_data_2024/effort-merged_data/dat_effort_", spname, ".rds"))
}
```

...
#FIXME: - For each diversity estimate; calculate a linear regression,
record slope, se, pvalue. - should we also set a minimum number of years
to be included to run the lm? - plot the results to view significant
increases, significant decreases, and non-signficant cell trends (map,
diverging histogram). Is there an overall direction of trend? Is there a
spatial signal in the results? What different might we learn from S,
Shan, Simp diversity? Is one of these better to pay attention to?
...
#Cell lat disappeared, so we will need to merge this back together or re-grid to plot again....
# add the cell locations back into the dataset
unique_cell_dat <- S_ERC_90sc %>%
  select(POLYFID, seqnum, cell_lon, geometry) %>%
  distinct()
...
#FIXME: add a plot for latitude, but need to first extract cell latitude from the geometry column, which is an sfc_POLYGON
```{r}
ggscatter(data=lm_S_results, x="cell_lon", y="slope", add="reg.line", alpha=0.25, col="sig") +
  ...
  scale_colour_manual(values = c("gray40", "darkturquoise")) + 
